...
function Scheduler(filesorglob, globoptions) {
    if (!(this instanceof Scheduler))
        return new Scheduler(filesorglob, globoptions);
    else {
        this.key = '@timer';
...
Scheduler.prototype.parse = function (filesorglob, globoptions) {
    var that = this;
...
Scheduler.prototype.parseComments = function (filepath, comments) {
var t = [];
...
Scheduler.prototype.isTimer = function (comment) {
    return this.testRegExp.test(comment.value);
};

Scheduler.prototype.getDateFromTimer = function (timer) {
